...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_p", "ln_gdppc", "avg_nsy", "avg_n", "avg_educ", "avg_gi", "avg_gx", "avg_nry"))
# Gi beskrivende navn til variablene (i samme rekkefølge som de ligger i datasettet)
labs <- c("Gjennomsnitlig arlig vekstrate i BNP pc 2000-2019 (%)", "Gjennomsnittlig arlig befolkningsvekst (%)", "BNP per innbygger", "Sparing som andel av BNI", "Gjennomsnittlig vekstrate i arbeidskraft", "Gjennomsnittlig antall ar i skole (15+)", "Gjennomsnittlig arlig vekstrate i investeringer", "Gjennomsnittlig arlig vekstrate i eksport", "Gjennomsnittlig arlig reduksjonsrate i naturressurser") 
...
plot2 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = ln_gdppc, na.rm = TRUE)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous(trans = 'log2', labels = dollar, breaks=c(500, 2000, 8000, 32000, 120000)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(-5, 0, 5, 10, 15, 20, 25)) # Velg vilke "ticks" som skal vises på x-akselen
plot2
...
plot3 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = ln_gdppc, na.rm = TRUE)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous(trans = 'log2', labels = dollar, breaks=c(500, 2000, 8000, 32000, 120000)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(2, 4, 6, 8, 10, 12)) # Velg vilke "ticks" som skal vises på x-akselen
plot3

plot4 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("nettosparing") + # Beskrivelse for x-akselen
  ylab("Arlig vekstrate i BNP") + # Beskrivelse for y-akselen
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
...
model1 <- lm(avg_gdpgrowth  ~ avg_educ + avg_p + ln_gdppc0 + avg_nsy + avg_n + avg_gi + avg_gx + avg_nry, data= no_outliers)
tab_model(model1)
